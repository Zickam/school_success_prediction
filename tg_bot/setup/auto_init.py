...
class AutoInitializer:
    ...
    async def _create_demo_class(self, school: School) -> Optional[Class]:
        ...
        try:
            class_ = Class(name="9A", school_uuid=school.uuid, start_year=2024)
    ...
    async def _create_demo_subjects(
        self, school: School, class_: Class
    ) -> Optional[list[Subject]]:

        try:
            subjects = [
                Subject(
                    name="Mathematics", school_uuid=school.uuid, class_uuid=class_.uuid
                ),
                Subject(
                    name="Physics", school_uuid=school.uuid, class_uuid=class_.uuid
                ),
                Subject(
                    name="Chemistry", school_uuid=school.uuid, class_uuid=class_.uuid
                ),
                Subject(
                    name="Biology", school_uuid=school.uuid, class_uuid=class_.uuid
                ),
                Subject(
                    name="English", school_uuid=school.uuid, class_uuid=class_.uuid
                ),
            ]
    ...
    async def _create_demo_teachers(
        self, school: School, subjects: list[Subject]
    ) -> Optional[list[User]]:
