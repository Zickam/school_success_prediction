...
async def create_school(
    school_data: SchoolCreate, session: AsyncSession = Depends(getSession)
):

    if school_data.director_uuid:
        result = await session.execute(
            select(User).where(
                User.uuid == school_data.director_uuid, User.role == Roles.director
            )
        )
...
async def get_school(school_id: UUID, session: AsyncSession = Depends(getSession)):
...
async def delete_school(school_id: UUID, session: AsyncSession = Depends(getSession)):
