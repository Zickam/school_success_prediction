...
teacher_subject = Table(
    ...
    Base.metadata,
    Column(
        "teacher_uuid", UUID(as_uuid=True), ForeignKey("user.uuid"), primary_key=True
    ),
    Column(
        "subject_uuid", UUID(as_uuid=True), ForeignKey("subject.uuid"), primary_key=True
    ),
)
...
parent_child = Table(
    ...
    Base.metadata,
    Column(
        "parent_uuid", UUID(as_uuid=True), ForeignKey("user.uuid"), primary_key=True
    ),
...
user_class = Table(
    ...
    Column(
        "class_uuid", UUID(as_uuid=True), ForeignKey("class.uuid"), primary_key=True
    ),
)
...
class User(Base):
    ...
    managed_class_uuid: Mapped[str] = mapped_column(
        UUID(as_uuid=True), ForeignKey("class.uuid"), nullable=True
    )
    school_uuid: Mapped[str] = mapped_column(
        UUID(as_uuid=True), ForeignKey("school.uuid"), nullable=True
    )
    ...
    classes: Mapped[List["Class"]] = relationship(
        secondary=user_class, back_populates="users"
    )
    grades: Mapped[List["Grade"]] = relationship(back_populates="user")
    managed_class: Mapped["Class"] = relationship(
        back_populates="homeroom_teacher", foreign_keys=[managed_class_uuid]
    )
...
class Subject(Base):
    ...
    name: Mapped[str] = mapped_column(String(100))
    school_uuid: Mapped[str] = mapped_column(
        UUID(as_uuid=True), ForeignKey("school.uuid")
    )
...
class Class(Base):
    ...
    homeroom_teacher: Mapped["User"] = relationship(
        back_populates="managed_class", foreign_keys="[User.managed_class_uuid]"
    )
    users: Mapped[List["User"]] = relationship(
        secondary=user_class, back_populates="classes"
    )
...
class Grade(Base):
    ...
    subject_uuid: Mapped[str] = mapped_column(
        UUID(as_uuid=True), ForeignKey("subject.uuid")
    )
