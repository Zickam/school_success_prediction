...
async def test_show_statistics(bot, dp):
    ...
    with patch("tg_bot.routers.user.welcome.make_api_request", new_callable=AsyncMock) as mock_api:
        ...
        # Verify the message was sent with correct text
        # Note: You'll need to mock bot.send_message to verify the exact text
...
async def test_show_statistics_success(mock_callback, bot):
    ...
    with patch("tg_bot.routers.user.welcome.make_api_request") as mock_request:
        ...
        # Verify API calls
        mock_request.assert_any_call("GET", f"/user?chat_id={mock_callback.from_user.id}")
...
async def test_show_statistics_user_not_found(mock_callback, bot):
    """Test statistics display when user not found"""
    with patch("tg_bot.routers.user.welcome.make_api_request") as mock_request:
...
async def test_show_statistics_stats_not_found(mock_callback, bot):
    """Test statistics display when statistics not found"""
    user_data = {
        "uuid": str(uuid4()),
        "role": "student",
        "name": "Test Student"
...
async def test_show_statistics_no_grades(mock_callback, bot):
    """Test statistics display when student has no grades"""
    user_data = {
        "uuid": str(uuid4()),
        "role": "student",
        "name": "Test Student"
    ...
    with patch("tg_bot.routers.user.welcome.make_api_request") as mock_request:
        ...
        assert "📊 Your Statistics:" in message_text
        assert "📚 Grades:" in message_text
        assert "Average Grade:" not in message_text  # Should not show average if no grades
        assert "📅 Attendance:" in message_text
        assert "90 days present" in message_text
        assert "8 days absent" in message_text
        assert "2 days late" in message_text
...
async def test_show_statistics_no_permission(mock_callback):
    """Test statistics display when user has no permission"""
    user_data = {
        "uuid": str(uuid4()),
        "role": "student",
        "name": "Test Student"
